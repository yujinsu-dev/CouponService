...
- **성과**
    - DB 비관적 락 기반 구조에서 nGrinder 1,000 VUser 조건까지 발급 정합성을 검증했으며, 1,500 VUser 이상부터 응답 지연과 실패가 증가하는 처리량 한계를 확인했습니다.
    - Redis Lua Script + DB row lock 기반 구조로 개선한 후, 1,000 VUser 기준 평균 응답 시간이 약 2,378ms에서 약 240ms로 약 90% 감소했고, TPS는 약 2.4배 개선되었습니다.
    - Redis 도입 후 1,000~4,000 VUser 구간에서는 모든 회차에서 중복 발급 없이 정상 처리되었고, 5,000 VUser 이상부터 일부 회차 실패가 발생하여 현재 로컬 테스트 환경 기준 안정 통과 구간을 4,000 VUser로 판단했습니다.
    - 최종 성공 여부는 nGrinder 성공 수뿐 아니라 DB 발급 건수, Redis 발급 건수, DB/Redis 잔여 재고를 교차 검증하여 정해진 수량만 발급됨을 확인했습니다.

| 항목 | DB 비관적 락 기반 | Redis Lua Script + DB row lock 기반 |
| --- | ---: | ---: |
